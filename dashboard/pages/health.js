  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
        <StatCard title="Healthy Backends" value={m.healthyBackends ?? 0} />
        <StatCard title="Unhealthy" value={m.unhealthyBackends ?? 0} />
        <StatCard title="Heap Used" value={`${((mem.heapUsed || 0) / 1048576).toFixed(1)} MB`} />
        <StatCard title="Redis" value={m.redis?.connected ? 'Connected' : 'Offline'} />
      </div>

      <section className="card">
        <p className="section-heading mb-4">Health Events</p>
        <ul className="space-y-0 font-mono text-sm max-h-96 overflow-y-auto">
          {events.length === 0 && <li className="empty-state">No events yet</li>}
          {events.map((ev, i) => (
            <li key={i} className="flex flex-col sm:flex-row sm:gap-4 table-row py-3 px-1">
              ...
              >
                {ev.type === 'recovery' ? 'Recovered' : 'Offline'}
              </span>
